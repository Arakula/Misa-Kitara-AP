...
#include "GUIDebug.h"
#include "LCDFrameBuffer.h"            /* Defines for first display */
...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex)
{
	...
	if(DisplayFlip)
	{
		location = (xres-1-x) * (bits_per_pixel / 8)  + (yres-1-y) * xres * (bits_per_pixel / 8);
	}
	else
	{
		location = x * (bits_per_pixel / 8)  + y * xres * (bits_per_pixel / 8);
	}
	//*(short*)(fbp + location) = (short)PixelIndex;
	*(unsigned long*)(fbp + location) = RGB((((unsigned short)PixelIndex)&0xF800)>>8,((unsigned short)PixelIndex&0x07E0)>>3,((unsigned short)PixelIndex&0x001F)<<3);
	return;
}
...
unsigned int LCD_L0_GetPixelIndex(int x, int y)
{
	...
	b = (*(unsigned long*)(fbp + location)&0x00FF0000)>>16;
	g = (*(unsigned long*)(fbp + location)&0x0000FF00)>>8;
	r = *(unsigned long*)(fbp + location)&0x000000FF;
	//PixelIndex = (short) *(fbp + location);
...
int  LCD_L0_Init(void) {
	xres = LCD_XSIZE;
	yres = LCD_YSIZE;
	bits_per_pixel = 32;//LCD_BITSPERPIXEL;
	return 0;
}
